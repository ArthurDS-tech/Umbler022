...
async function testWebhookNoAuth() {
  console.log('🧪 TESTE WEBHOOK SEM AUTENTICAÇÃO');
  console.log('==================================\n');
  ...
  try {
    ...
    // 2. Testar endpoint de teste (que não requer autenticação)
    console.log('\n2️⃣ Testando endpoint de teste...');
    try {
      const testResponse = await axios.get('http://localhost:3000/webhook/test', { timeout: 5000 });
      console.log('✅ Endpoint de teste funcionando');
      console.log('📊 Resposta:', testResponse.data);
    } catch (err) {
      console.log('❌ Endpoint de teste não funcionando:', err.message);
    }

    // 3. Testar webhook com assinatura falsa
    console.log('\n3️⃣ Testando webhook com assinatura falsa...');
    try {
      const response = await axios.post('http://localhost:3000/webhook/umbler', realWebhookPayload, {
        headers: {
          'Content-Type': 'application/json',
          'User-Agent': 'Umbler-Webhook-Test',
          'x-umbler-signature': 'fake-signature-for-testing'
        },
        timeout: 10000
      });
      ...
      console.log('❌ Erro ao enviar webhook:', err.message);
      if (err.response) {
